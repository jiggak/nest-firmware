...
(cd u-boot/u-boot
   ...
   DEFINES='-DNEST_BUILD_CONFIG=\"jiggak@github\"'
   make ARCH=arm CROSS_COMPILE=$TOOLCHAIN_PREFIX distclean
   make ARCH=arm CROSS_COMPILE=$TOOLCHAIN_PREFIX EXTRA_CPPFLAGS=$DEFINES j49
   make ARCH=arm CROSS_COMPILE=$TOOLCHAIN_PREFIX EXTRA_CPPFLAGS=$DEFINES

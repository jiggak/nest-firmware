...
CONNMAN_LEGACY_VERSION = 1.30
CONNMAN_LEGACY_SOURCE = connman-$(CONNMAN_LEGACY_VERSION).tar.xz
CONNMAN_LEGACY_SITE = $(BR2_KERNEL_MIRROR)/linux/network/connman
CONNMAN_LEGACY_DEPENDENCIES = libglib2 dbus iptables-legacy
CONNMAN_LEGACY_INSTALL_STAGING = YES
CONNMAN_LEGACY_LICENSE = GPLv2
CONNMAN_LEGACY_LICENSE_FILES = COPYING
CONNMAN_LEGACY_CONF_OPTS += \
...
CONNMAN_LEGACY_CONF_OPTS += --enable-client
CONNMAN_LEGACY_DEPENDENCIES += readline

define CONNMAN_LEGACY_INSTALL_CM
	$(INSTALL) -m 0755 -D $(@D)/client/connmanctl $(TARGET_DIR)/usr/bin/connmanctl
endef

CONNMAN_LEGACY_POST_INSTALL_TARGET_HOOKS += CONNMAN_LEGACY_INSTALL_CM
else
CONNMAN_LEGACY_CONF_OPTS += --disable-client
endif
